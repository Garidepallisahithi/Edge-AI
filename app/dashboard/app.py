...
def safety_readiness_score(metrics: dict, uncertainty: dict) -> float:
    """
    Lightweight readiness score for demo use only.
    """
...
def roc_pr_figures(df: pd.DataFrame, label_col: str = "label", prob_col: str = "anomaly_probability"):
    if df.empty or label_col not in df.columns or prob_col not in df.columns:
        return None, None
...
def reliability_figure(y_true: np.ndarray, y_prob: np.ndarray):
...
def confidence_histogram(df: pd.DataFrame, conf_col: str = "confidence"):
    if df.empty or conf_col not in df.columns:
        return None
    fig = px.histogram(df, x=conf_col, nbins=25, title="Confidence Distribution")
    fig.update_layout(height=420, margin=dict(l=20, r=20, t=50, b=20))
    return fig


def feature_importance_fig(df: pd.DataFrame, top_n: int = 20):
    if df.empty or "feature" not in df.columns or "importance" not in df.columns:
        return None
    top = df.sort_values("importance", ascending=False).head(top_n).sort_values("importance")
    fig = px.bar(
        top,
        x="importance",
        y="feature",
...
def local_explanation_fig(df: pd.DataFrame, sample_id):
    ...
    fig = px.bar(
        sdf.sort_values("weighted_score", ascending=True),
        x="weighted_score",
        y="feature",
        orientation="h",
...
def threshold_preview(df: pd.DataFrame, threshold: float) -> dict:
    if df.empty or "anomaly_probability" not in df.columns:
        return {"predicted_abnormal": 0, "predicted_normal": 0, "f1": None}
...
uncertainty = load_json(str(UNCERTAINTY_JSON))
explain_summary = load_json(str(EXPLAIN_SUMMARY_JSON))

split_df = load_csv(str(SPLIT_CSV))
validation_df = load_csv(str(VALIDATION_PRED_CSV))
internal_df = load_csv(str(INTERNAL_PRED_CSV))
official_df = load_csv(str(OFFICIAL_PRED_CSV))
global_imp = load_csv(str(GLOBAL_IMPORTANCE_CSV))
local_exp = load_csv(str(LOCAL_EXPLANATIONS_CSV))

best_threshold = float(metrics.get("best_threshold", 0.51) or 0.51)
default_conf_cutoff = float(uncertainty.get("confidence_cutoff", 0.80) or 0.80)
readiness_score = safety_readiness_score(metrics, uncertainty)
...
confidence_cutoff = st.sidebar.slider(
    "Confidence cutoff",
    min_value=0.50,
    max_value=1.0,
    value=float(default_conf_cutoff),
    step=0.01,
)
...
if not local_exp.empty and "sample_id" in local_exp.columns:
    sample_choices = sorted(local_exp["sample_id"].astype(str).unique().tolist())

sample_id = None
if sample_choices:
    # --- sample selector: default to most anomalous sample ---
    sample_priority = (
    ...
    sample_choices = sample_priority["sample_id"].astype(str).tolist()
    sample_id = st.sidebar.selectbox(
        "Sample for local explanation",
        options=sample_choices,
        index=0,
        key="sample_for_local_explanation",
    )
    # ---------------------------------------------------------
    ...
    for label, path in [
        ...
        status_html = "<span class='check-ok'>OK</span>" if path.exists() else "<span class='check-miss'>Missing</span>"
        st.sidebar.markdown(f"- {label}: {status_html}", unsafe_allow_html=True)

    st.sidebar.markdown("---")
    st.sidebar.markdown(
        f"**Phase-5 readiness score:** `{readiness_score:.1f}/100`"
    )
    st.sidebar.markdown("---")
...
# Hero
# ----------------------------
status_text = "READY FOR DEMO" if readiness_score >= 85 else "NEEDS REVIEW"
st.markdown(
    f"""
<div class="hero">
...
# KPI row
# ----------------------------
internal_metrics = metrics.get("internal_test", {})
validation_metrics = metrics.get("validation", {})

kpi1, kpi2, kpi3, kpi4, kpi5 = st.columns(5)
with kpi1:
    metric_card("Internal Test F1", fmt(internal_metrics.get("f1", 0.0)))
with kpi2:
    metric_card("ROC-AUC", fmt(internal_metrics.get("roc_auc", 0.0)))
with kpi3:
    metric_card("PR-AUC", fmt(internal_metrics.get("pr_auc", 0.0)))
with kpi4:
...
with s2:
    metric_card("Confidence Cutoff", fmt(confidence_cutoff), "Controls selective accuracy")
with s3:
...
with tabs[0]:
    ...
    with c1:
        ...
        st.markdown(
            """
- **Goal:** detect battery thermal anomalies early.
...
"""
        ...
        with x1:
            metric_card("Detect", "Yes", "Abnormal samples are ranked")
        with x2:
            metric_card("Explain", "Yes", "Top contributing features")
        with x3:
            metric_card("Trust", "Yes", "Confidence + calibration")

        st.markdown("### Internal split summary")
        if not split_df.empty and "internal_split" in split_df.columns:
            ...
            fig = px.bar(
                ...
                y="count",
                color="split",
                title="Stratified Internal Split Size",
            )
            fig.update_layout(height=360, margin=dict(l=20, r=20, t=50, b=20), showlegend=False)
            st.plotly_chart(fig, use_container_width=True)
        else:
            st.info("Split file not found. Add data/processed/mendeley_internal_split.csv for dataset summary.")

    with c2:
        st.markdown("### Readiness Checklist")
        checklist = [
            ("Dataset acquired", OFFICIAL_PRED_CSV.exists() or INTERNAL_PRED_CSV.exists()),
        ...
        for name, ok in checklist:
            status_html = "<span class='check-ok'>OK</span>" if ok else "<span class='check-miss'>Missing</span>"
            st.markdown(f"- {name}: {status_html}", unsafe_allow_html=True)
        ...
        status_color = "#16a34a" if readiness_score >= 85 else "#eab308"
        st.markdown(
            f"""
...
with tabs[1]:
    ...
    with p1:
        metric_card("Validation Accuracy", fmt(validation_metrics.get("accuracy", 0.0)))
    with p2:
        metric_card("Validation F1", fmt(validation_metrics.get("f1", 0.0)))
    with p3:
        metric_card("Internal Test F1", fmt(internal_metrics.get("f1", 0.0)))
    ...
    if not internal_df.empty and "label" in internal_df.columns and "anomaly_probability" in internal_df.columns:
        ...
        y_pred = internal_df["prediction"].astype(int).values if "prediction" in internal_df.columns else (
            y_prob >= threshold
        ).astype(int)
        ...
        with left:
            fig = confusion_heatmap(cm, "Internal Test Confusion Matrix")
            st.plotly_chart(fig, use_container_width=True)

        with right:
            roc_fig, pr_fig = roc_pr_figures(internal_df, "label", "anomaly_probability")
            if roc_fig is not None and pr_fig is not None:
                st.plotly_chart(roc_fig, use_container_width=True)
                st.plotly_chart(pr_fig, use_container_width=True)
            else:
                st.warning("ROC/PR curves cannot be built because internal test labels are missing or single-class.")
        ...
        st.warning("Internal test predictions are missing. The performance tab needs internal_test_predictions_calibrated.csv.")
...
with tabs[2]:
    ...
    with u5:
        metric_card(
            "Coverage",
            pct(uncertainty.get("coverage_at_confidence_cutoff", 0.0)),
            "Fraction of predictions kept above confidence cutoff",
        )
    with u6:
        metric_card(
            "Selective F1",
            fmt(uncertainty.get("selective_f1_at_confidence_cutoff", 0.0)),
            "Performance among confident predictions",
        )
    ...
    if not internal_df.empty and "confidence" in internal_df.columns:
        ...
        with c1:
            fig = confidence_histogram(internal_df, "confidence")
            if fig is not None:
                st.plotly_chart(fig, use_container_width=True)

        with c2:
            if "label" in internal_df.columns and "anomaly_probability" in internal_df.columns:
                y_true = internal_df["label"].astype(int).values
                y_prob = np.clip(internal_df["anomaly_probability"].astype(float).values, 1e-6, 1 - 1e-6)
                rel = reliability_figure(y_true, y_prob)
                st.plotly_chart(rel, use_container_width=True)
            else:
                st.warning("Reliability curve cannot be computed because labels or probabilities are missing.")

        low_conf = internal_df.sort_values("confidence", ascending=True).head(15)
        st.markdown("### Lowest-confidence samples")
        st.dataframe(low_conf, use_container_width=True, hide_index=True)
    else:
        st.warning("Internal confidence data is missing.")
...
with tabs[3]:
    ...
    with e1:
        if not global_imp.empty:
            fig = feature_importance_fig(global_imp, top_n=20)
            if fig is not None:
                st.plotly_chart(fig, use_container_width=True)
            else:
                st.warning("Global importance file is present but does not contain feature/importance columns.")
        else:
    ...
    if explain_summary:
        ...
        st.info("No explainability summary file found.")

    if not local_exp.empty:
        st.markdown("### Top anomalous local explanations")
        top_local = local_exp.sort_values("weighted_score", ascending=False).head(15)
        st.dataframe(top_local, use_container_width=True, hide_index=True)
...
with tabs[4]:
    ...
    with left:
        ...
        if not official_df.empty:
            if "anomaly_probability" in official_df.columns:
                off = official_df.copy()
                off["prediction_at_threshold"] = (off["anomaly_probability"] >= threshold).astype(int)
                off["confidence"] = np.maximum(
                    off["anomaly_probability"].astype(float),
                    1 - off["anomaly_probability"].astype(float),
                )
                ...
                st.warning("Official predictions file is missing the anomaly_probability column.")
        else:
            st.warning("official_test_predictions.csv is missing or empty.")
    ...
    if not internal_df.empty and "label" in internal_df.columns and "anomaly_probability" in internal_df.columns:
        ...
        for thr in np.linspace(0.05, 0.95, 91):
            ...
            sweep.append(
                {
                    "threshold": thr,
                    "precision": __import__("sklearn.metrics").metrics.precision_score(y_true, pred, zero_division=0),
                    "recall": __import__("sklearn.metrics").metrics.recall_score(y_true, pred, zero_division=0),
        ...
        fig = px.line(
            ...
            title="Threshold Sweep on Internal Test",
        )
        fig.update_layout(height=420, margin=dict(l=20, r=20, t=50, b=20))
        st.plotly_chart(fig, use_container_width=True)
    else:
        st.info("Threshold sweep requires internal labels and probabilities.")
